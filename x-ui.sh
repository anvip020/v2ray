...
update() {
    ...
    bash <(curl -Ls https://github.com/anvip020/x-ui/main/install.sh)
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://raw.githubusercontent.com/anvip020/x-ui/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
