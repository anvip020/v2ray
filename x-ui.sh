...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/anvip020/x-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/anvip020/x-ui/blob/main/install.sh)
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/anvip020/x-ui/blob/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
